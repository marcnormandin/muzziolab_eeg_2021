function [settings] = men_spindledetection_load_settings(wakeCode, nremCode, remCode)

% Choose default command line output for Spindle_detect_v17_4s_GUI
%settings.output = hObject;
...
settings.endTime = 17940; %3600;% End time for analysis (s) e.g. end of first hr=3600, second hr=7200, third hr=10800
settings.numblock = 5; % Enter number of blocks within the selected interval to be analized (e.g. For analyzing each hour of selected 4hr recording, numblock=4. For analyzing the entire length of selected 4hr recording, numblock=1.)
...
settings.spindleInterval = 0.1; % minimum time between spindles (seconds) (default 0.5)
settings.threshState = 'all'; % Specify which sleep/wake state data to use for threshold setting, 'all'(default), 'wake','nrem','rem'
settings.lower_threshRatio = 1.2; % (default 1.2)
settings.upper_threshRatio = 3.5; % (default 3.5)

...
function [data,header,cfg] = men_edf_read(filename)
...
while strcmp(hdr.channelname(m,1:15),'EDF Annotations')
        ...
        if size(numberstops,2) > 1
            for j = 2:size(numberstops,2)
                ...
                elseif size(tmp,1) > 1
                    ...
                    header.events.DUR = [header.events.DUR 1];
                    header.events.TYP = [header.events.TYP cellstr('EVENT')];
                end
            end
        end
    end
    ...
    % Marc because the code has a logic error
    if m == 0
        break;
    end
end
